...
# Create a non-root user named 'varshith' with a home directory
RUN useradd -m varshith
...
RUN mkdir logs qr_codes && chown varshith:varshith logs qr_codes

# Copy the rest of the application's source code into the container, setting ownership to 'varshith'
COPY --chown=varshith:varshith . .

# Switch to the 'varshith' user to run the application
USER varshith
...
CMD ["--url","https://github.com/varshith-29"]